class Address < ApplicationRecord
  ...
  validates :zipcode, format: { with: /\A\d{3}[-]\d{4}$|^\d{3}[-]\d{2}$|^\d{3}\z/}, presence: true, on: :validates_step3
  validates :prefecture, presence: true, numericality: { less_than_or_equal_to: 47 } , on: :validates_step3
